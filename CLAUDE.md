...
### 評価関数の種類
- **evaluate_scene_002**: 基本評価（5項目）
- **evaluate_scene_003**: シーン別評価（5項目）
- **evaluate_scene_004**: 詳細評価（5項目）
- **evaluate_scene_005**: 総合評価（5項目）

### 評価基準（全関数共通：5項目固定）
1. **誠実性・正確性** - 正しい情報提供、嘘や誤魔化しの排除
2. **傾聴力** - 顧客の話を遮らない、共感の表現
3. **適応性** - 顧客ニーズに応じた柔軟な対応
4. **プロフェッショナリズム** - 敬語・マナー・身だしなみ
5. **問題解決能力** - 顧客の課題を理解し解決策を提案

## 最近の変更履歴

### 2025年1月9日
- **評価システムの統一化**: 全評価関数を5項目評価に統一
- **Migration追加**: `20250809002000_fix_all_scenes_to_5_criteria.sql`で既存データを5項目形式に修正
- **UI改善**: 
  - ダッシュボード配色統一（背景/#0F172A、カード/#1E293B、アクセント#7C4DFF/#F59E0B）
  - ダッシュボード主要KPIの集約とレイアウト改善（左右55:45）
  - 評価画面の3分割均等配置とグラフ75%拡大
  - 最新録音の表示改善（7件表示、スコア色分け、ホバープレビュー）
  - Reflection Notesの自動保存機能とトースト通知追加
- **データ整合性**: philosophy（V/M/P）の常時表示、自動シード機能とポーリング実装
- **グラフ表示**: 基本スキルとシーン別スキルのチャートサイズ統一
