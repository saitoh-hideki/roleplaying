-- クラウド版Supabase用の安全な修正SQL
-- Supabase DashboardのSQL Editorで実行してください

-- 1. evaluation_criteriaテーブルにtypeカラムを追加（エラーを無視）
DO $$ 
BEGIN
    IF NOT EXISTS (
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tgname = 'update_scene_evaluation_criteria_updated_at'
    ) THEN
