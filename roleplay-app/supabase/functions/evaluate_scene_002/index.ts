...
**重要**: 評価結果には必ず以下のシーン002特有の評価項目を含めてください：
- 予約確認
- スムーズな案内
- 追加提案
- 時間配分
...
**必ず以下のシーン002特有の評価項目を含めて評価してください：**
- 予約確認: 事前予約の内容を正確に確認できているか
- スムーズな案内: 予約に基づいた効率的な案内ができているか
- 追加提案: 予約内容以外の追加サービスを適切に提案できているか
- 時間配分: 予約時間内で適切な対応ができているか
- お客様満足度: 予約していただいた会員様に満足いただける対応ができているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン002の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    if (sceneFeedbackNotes.length < sceneCriteria.length) {
      ...
      for (const missingCriterion of missingCriteria) {
        ...
        sceneFeedbackNotes.push({
          ...
          score: 1, // デフォルトで1点
          comment: `シーン002の評価項目「${missingCriterion.criterion_name}」について評価されました。`
        })
      }
    }
