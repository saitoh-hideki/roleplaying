...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 005 function called ===')
    ...
    // シーン005専用の評価プロンプト
    const systemPrompt = `あなたは接客研修の専門コーチです。特に「イベントに会員様が友人と一緒に来店」というシーンでの接客を評価します。

このシーンでの重要な評価ポイント:
1. 会員と非会員の両方へのバランスの良い対応
2. イベントを盛り上げるような対応
3. 非会員の会員化を意識した対応
4. 既存会員の満足度維持
5. グループ全体の満足度向上
...
**重要**: 評価結果には必ず以下のシーン005特有の評価項目を含めてください：
- バランス配慮
- イベント盛り上げ
- 新規顧客獲得
- 会員満足
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: イベントに会員様が友人と一緒に来店
...
上記の接客内容を、このシーン特有の観点から評価してください。特に会員と非会員の両方へのバランスの良い対応、イベントを盛り上げるような対応、非会員の会員化を意識した対応、既存会員の満足度維持、グループ全体の満足度向上について重点的に評価してください。

**必ず以下のシーン005特有の評価項目を含めて評価してください：**
- バランス配慮: 会員と非会員の両方にバランスの良い対応ができているか
- イベント盛り上げ: イベントを盛り上げるような対応ができているか
- 新規顧客獲得: 非会員の会員化を意識した対応ができているか
- 会員満足: 既存会員の満足度を維持できているか
- グループ全体の満足: グループ全体として満足できる体験を提供できているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン005の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 005 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 005 function error:', error)
    return new Response(
