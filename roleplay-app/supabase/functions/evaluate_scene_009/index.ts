...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 009 function called ===')
...
**重要**: 評価結果には必ず以下のシーン009特有の評価項目を含めてください：
- 期待値管理
- 紹介者への配慮
- 信頼関係の構築
- 具体的提案
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: 紹介で来店したお客様
...
上記の接客内容を、このシーン特有の観点から評価してください。特に期待値の高い非会員への接客、紹介者への感謝の表現、紹介者の信頼に応える対応、非会員への適切な情報提供について重点的に評価してください。

**必ず以下のシーン009特有の評価項目を含めて評価してください：**
- 期待値管理: 紹介で来店されたお客様の期待値に適切に対応できているか
- 紹介者への配慮: 紹介者への配慮と感謝の気持ちが表現できているか
- 信頼関係の構築: 初回でも信頼関係を構築できる対応ができているか
- 具体的提案: お客様の状況に応じた具体的な提案ができているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン009の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 009 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 009 function error:', error)
    return new Response(
