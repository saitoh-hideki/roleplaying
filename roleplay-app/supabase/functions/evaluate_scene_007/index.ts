...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 007 function called ===')
    ...
    // シーン007専用の評価プロンプト
    const systemPrompt = `あなたは接客研修の専門コーチです。特に「会員を辞めたいと来店」というシーンでの接客を評価します。
...
**重要**: 評価結果には必ず以下のシーン007特有の評価項目を含めてください：
- 感情への配慮
- 理由の把握
- 解決策の提示
- 関係性の修復
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: 会員を辞めたいと来店
...
上記の接客内容を、このシーン特有の観点から評価してください。特にネガティブな目的への適切な対応、会員様の気持ちへの共感、退会理由の丁寧な確認、継続利用の可能性の探りについて重点的に評価してください。

**必ず以下のシーン007特有の評価項目を含めて評価してください：**
- 感情への配慮: 辞めたいという感情に適切に配慮できているか
- 理由の把握: 辞めたい理由を深く理解できているか
- 解決策の提示: 辞めずに済む解決策を提示できているか
- 関係性の修復: 関係性を修復するような対応ができているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン007の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 007 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 007 function error:', error)
    return new Response(
