...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 008 function called ===')
    ...
    // シーン008専用の評価プロンプト
    const systemPrompt = `あなたは接客研修の専門コーチです。特に「SmartLife AO校に興味を示す会員様」というシーンでの接客を評価します。

このシーンでの重要な評価ポイント:
1. 前向きだがまだ不安がある会員様への対応
2. SmartLife AO校の魅力の適切な説明
3. 不安要素への理解と安心感の提供
4. 具体的な情報提供と次のステップの提案
5. 会員様の将来への配慮

評価項目：
${allCriteria.map((criterion, index) => `${index + 1}. ${criterion.label}: ${criterion.description}`).join('\n')}

**重要**: 評価結果には必ず以下のシーン008特有の評価項目を含めてください：
- 興味の維持
- 不安の解消
- 具体的説明
- 次のアクション
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: SmartLife AO校に興味を示す会員様

状況説明:
- 前向きだがまだ不安がある会員様への接客
- SmartLife AO校の魅力の説明が必要
- 不安要素への理解と安心感の提供
- 具体的な情報提供と次のステップの提案
- 会員様の将来への配慮
...
上記の接客内容を、このシーン特有の観点から評価してください。特に前向きだがまだ不安がある会員様への対応、SmartLife AO校の魅力の適切な説明、不安要素への理解と安心感の提供、具体的な情報提供と次のステップの提案について重点的に評価してください。

**必ず以下のシーン008特有の評価項目を含めて評価してください：**
- 興味の維持: お客様の興味を維持・高める対応ができているか
- 不安の解消: お客様の不安を適切に解消できているか
- 具体的説明: SmartLife AO校について具体的で分かりやすく説明できているか
- 次のアクション: 具体的な次のステップを提示できているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン008の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 008 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 008 function error:', error)
    return new Response(
