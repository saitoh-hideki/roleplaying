...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 006 function called ===')
    ...
    // シーン006専用の評価プロンプト
    const systemPrompt = `あなたは接客研修の専門コーチです。特に「DMを送った会員様に電話をする」というシーンでの接客を評価します。
...
**重要**: 評価結果には必ず以下のシーン006特有の評価項目を含めてください：
- 声かけの自然さ
- 価値提供
- 反応への対応
- 次のステップ
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: DMを送った会員様に電話をする
...
上記の接客内容を、このシーン特有の観点から評価してください。特にアウトバウンド型の声かけの適切性、DMの内容への言及、電話での親しみのある対応、目的の明確化について重点的に評価してください。

**必ず以下のシーン006特有の評価項目を含めて評価してください：**
- 声かけの自然さ: 自然で押しつけがましくない声かけができているか
- 価値提供: お客様にとって価値のある情報を提供できているか
- 反応への対応: お客様の反応に適切に対応できているか
- 次のステップ: 次のアクションを明確に提示できているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン006の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 006 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 006 function error:', error)
    return new Response(
