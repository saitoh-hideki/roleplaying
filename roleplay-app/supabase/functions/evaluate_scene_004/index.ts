...
serve(async (req: Request) => {
  ...
  try {
    console.log('=== Evaluate Scene 004 function called ===')
    ...
    // シーン004専用の評価プロンプト
    const systemPrompt = `あなたは接客研修の専門コーチです。特に「初回訪問のお客様への対応」というシーンでの接客を評価します。
...
**重要**: 評価結果には必ず以下のシーン004特有の評価項目を含めてください：
- 第一印象
- マナー
- 目的の明確化
- 環境適応
...
評価結果は以下のJSON形式で返してください：
{
  ...
  "criteriaScores": [
    ...
    const userPrompt = `シーン: 初めて会員様のお宅へ訪問
...
上記の接客内容を、このシーン特有の観点から評価してください。特に訪問時の第一印象、プライベート空間への配慮、会員様の自宅での安心感の提供、適切な距離感の維持について重点的に評価してください。

**必ず以下のシーン004特有の評価項目を含めて評価してください：**
- 第一印象: 訪問時の第一印象が良く、信頼感を与えられているか
- マナー: 訪問時の適切なマナーと礼儀正しさができているか
- 目的の明確化: 訪問の目的を明確に伝えられているか
- 環境適応: お客様の環境に適応した対応ができているか
- アフターサポート: 解決後のアフターサポートや継続的な関係構築ができているか`
    ...
    if (sceneFeedbackNotes.length === 0 && sceneCriteria.length > 0) {
      ...
      for (const sceneCriterion of sceneCriteria) {
        sceneFeedbackNotes.push({
          ...
          comment: `シーン004の評価項目「${sceneCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    if (sceneFeedbackNotes.length < sceneCriteria.length) {
      ...
      for (const missingCriterion of missingCriteria) {
        ...
        sceneFeedbackNotes.push({
          ...
          score: 1, // デフォルトで1点
          comment: `シーン004の評価項目「${missingCriterion.criterion_name}」について評価されました。`
        })
      }
    }
    ...
    console.log('Scene 004 evaluation completed successfully')
    return new Response(
      JSON.stringify({ 
    ...
    console.error('Evaluate Scene 004 function error:', error)
    return new Response(
