...
import { mockRecordings, mockScenes } from '../utils/mockData';
...
export default function HistoryScreen({ navigation }: any) {
  ...
  const getFilteredRecordings = () => {
    switch (selectedFilter) {
      case 'recent':
        return mockRecordings.slice(0, 5);
      case 'completed':
        return mockRecordings.filter(r => r.transcript);
      case 'pending':
        return mockRecordings.filter(r => !r.transcript);
      default:
        ...
        <View style={styles.statsContainer}>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statNumber}>
              {mockRecordings.filter(r => r.transcript).length}
            </Text>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statNumber}>
              {Math.round(
                mockRecordings.reduce((acc, r) => acc + r.duration, 0) / 60
              )}
            </Text>
            <Text style={styles.statLabel}>総練習時間(分)</Text>
          </View>
        </View>
                ...
                <View style={styles.recordingHeader}>
                  <View style={styles.sceneInfo}>
                    <Text style={styles.sceneIcon}>
                      {getSceneIcon(recording.sceneId)}
                    </Text>
                    <View style={styles.sceneDetails}>
                      <Text style={styles.sceneTitle}>
                        {getSceneTitle(recording.sceneId)}
                      </Text>
                  ...
                  <View style={styles.recordingMeta}>
                    ...
                    <View
                      style={[
                        styles.statusBadge,
                        {
                          backgroundColor: recording.transcript
                            ? '#10B98120'
                            : '#F59E0B20',
                        },
                      ]}
                    >
                      <Ionicons
                        name={recording.transcript ? 'checkmark-circle' : 'hourglass'}
                        size={14}
                        color={recording.transcript ? '#10B981' : '#F59E0B'}
                      />
                      <Text
                        style={[
                          styles.statusText,
                ...
                <View style={styles.recordingFooter}>
                  ...
                  {recording.transcript && (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        // 再録音の処理
                        navigation.navigate('Record', { sceneId: recording.sceneId });
                      }}
                    >
